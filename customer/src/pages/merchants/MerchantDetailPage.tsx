...
import { Heart, MapPin, Star, Clock, Globe, ChevronLeft, Loader2 } from 'lucide-react'
import { publicApi } from '@/api/endpoints/public'
import { favouritesApi } from '@/api/endpoints/profile'
...
type Tab = 'coupons' | 'stores' | 'reviews'
...
export default function MerchantDetailPage() {
  ...
  const { data: favData } = useQuery({
    ...
    queryFn: () => favouritesApi.check(merchantId).then((r) => r.data.data?.is_favourite ?? false),
    ...
    <div className="max-w-2xl mx-auto pb-8">
      ...
      <div className="relative h-48 bg-gradient-to-br from-brand-100 to-purple-100 overflow-hidden">
        {merchant.business_logo ? (
          <img src={merchant.business_logo} alt={merchant.business_name} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="px-4 mt-5 flex bg-gray-100 rounded-xl p-1 gap-1">
        {(['coupons', 'stores', 'reviews'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
