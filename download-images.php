 ...
 * Downloads Unsplash images to local storage and updates the database.
 *
...
function updateDB(string $table, string $column, array $updates): void {
    // Build:  UPDATE table SET column = CASE id WHEN x THEN y ... ELSE column END WHERE id IN (...)
    $db   = getDB();
    ...
    $sql    = "UPDATE `$table` SET `$column` = CASE `id` $when ELSE `$column` END WHERE `id` IN ($inList)";
    $affected = $db->exec($sql);
    echo "  Updated $affected row(s) in $table.$column\n";
}

// ── Create directories ────────────────────────────────────────────────────────
echo "\n[1/4] Creating upload directories...\n";
ensureDir(LOGO_DIR);
ensureDir(GALLERY_DIR);
ensureDir(ADS_DIR);

// ── Merchant Logos ───────────────────────────────────────────────────────────
echo "\n[2/4] Downloading merchant logos...\n";
...
foreach ($logos as $item) {
    ...
    downloadImage($item['url'], LOGO_DIR . '/' . $filename);
    $logoUpdates[] = ['id' => $item['id'], 'value' => '/uploads/logos/' . $filename];
}

// ── Store Gallery ─────────────────────────────────────────────────────────────
echo "\n[3/4] Downloading gallery images...\n";
...
foreach ($gallery as $item) {
    ...
    downloadImage($item['url'], GALLERY_DIR . '/' . $filename);
    $galleryUpdates[] = ['id' => $item['id'], 'value' => '/uploads/gallery/' . $filename];
}

// ── Advertisements ────────────────────────────────────────────────────────────
echo "\n[4/4] Downloading advertisement images...\n";
...
foreach ($ads as $item) {
    ...
    downloadImage($item['url'], ADS_DIR . '/' . $filename);
    $adUpdates[] = ['id' => $item['id'], 'value' => '/uploads/ads/' . $filename];
}

// ── Update Database ───────────────────────────────────────────────────────────
echo "\n[DB] Updating merchant logos...\n";
updateDB('merchants', 'business_logo', $logoUpdates);

echo "[DB] Updating store gallery...\n";
updateDB('store_gallery', 'image_url', $galleryUpdates);

echo "[DB] Updating advertisements...\n";
updateDB('advertisements', 'media_url', $adUpdates);
...
echo "\nDone!\n";
echo "  Logos   : $logoCount files in " . LOGO_DIR    . "\n";
echo "  Gallery : $galleryCount files in " . GALLERY_DIR . "\n";
echo "  Ads     : $adsCount files in " . ADS_DIR     . "\n";
